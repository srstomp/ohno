 ...
 *
 * Read-only view of the ohno tasks database for the `ohno serve` HTTP UI.
 * Uses Node's built-in node:sqlite (DatabaseSync) in readonly mode.
 */
...
export async function exportDatabase(dbPath: string): Promise<KanbanData> {
  ...
  try {
    ...
    data.projects = db.prepare("SELECT * FROM projects").all() as unknown[];
    data.epics = db.prepare("SELECT * FROM epics").all() as unknown[];
    ...
    data.task_dependencies = db.prepare(`
